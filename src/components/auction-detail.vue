  ...
  <transition name="slide">
    <div id="auction-detail">
      ...
      <div class="main">
        ...
        <div class="auction-banner" v-if="auctionDetail.cover">
          <img :src="prefix+auctionDetail.cover" />
        </div>
        ...
        <div class="auction-detail-3">
          <div class="row">
            ...
            <div class="icon-location" @click="openLocation"></div>
          </div>
          <div class="row">
        ...
        <div class="auction-detail-5">
          ...
          <div class="picture-list">
            <div class="picture" :key="$index" v-for="(item,$index) in auctionDetail.files">
              <img v-lazy="prefix+item.thumb2FileName" @click="showViewer($index, $event)" />
            </div>
          </div>
        </div>
      ...
      <md-image-viewer v-model="isViewerShow" :list="files" :has-dots="true" :initial-index="viewerIndex">
      </md-image-viewer>
    </div>
...
import { Toast, ImageViewer } from "mand-mobile";
...
export default {
  ...
  methods: {
        ...
        assetDetail(params).then(res => {
          if (res.code == 0) {
            this.$nextTick(() => {
              ...
              if (this.auctionDetail.files && this.auctionDetail.files.length) {
                let files = [];
                this.auctionDetail.files.forEach(element => {
                  files.push(this.prefix+element.thumb2FileName);
                });
                this.files = files;
              }
    ...
    openLocation(){
      let longitude = this.auctionDetail.longitude
      let latitude = this.auctionDetail.latitude
      let locationName = this.auctionDetail.locationName
      if(longitude&&letitude){
